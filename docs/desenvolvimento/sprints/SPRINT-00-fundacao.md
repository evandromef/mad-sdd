# SPRINT-00 - Fundacao

## Objetivo da sprint

Preparar a base tecnica do projeto para desenvolvimento seguro e repetivel.

## Periodo

- Inicio: 2026-07-10
- Fim:

## Epicos relacionados

- EPICO-01 - Fundacao Tecnica

## Escopo planejado

- Estrutura backend em `codebase/backend/`.
- Estrutura frontend em `codebase/frontend/`.
- PostgreSQL 17 e Flyway.
- Docker local.
- CI inicial.
- Padroes de testes e OpenAPI.

## Tasks

| Task | Descricao | Status | Responsavel |
| --- | --- | --- | --- |
| [TASK-0001](../tasks/sprint-00/TASK-0001-scaffold-backend-spring-boot.md) | Scaffold do backend Spring Boot. | Concluida | Codex |
| [TASK-0002](../tasks/sprint-00/TASK-0002-scaffold-frontend-angular.md) | Scaffold do frontend Angular. | Concluida | Codex |
| [TASK-0003](../tasks/sprint-00/TASK-0003-banco-flyway-primeira-migration.md) | Banco local, Flyway e primeira migration. | Em revisao | Codex |
| [TASK-0004](../tasks/sprint-00/TASK-0004-docker-compose-desenvolvimento.md) | Docker Compose de desenvolvimento. | Planejada | |
| [TASK-0005](../tasks/sprint-00/TASK-0005-openapi-contrato-inicial-api.md) | OpenAPI e contrato inicial da API. | Planejada | |
| [TASK-0006](../tasks/sprint-00/TASK-0006-padroes-testes-cobertura.md) | Padroes de testes e cobertura. | Planejada | |
| [TASK-0007](../tasks/sprint-00/TASK-0007-ci-inicial.md) | CI inicial. | Planejada | |
| [TASK-0008](../tasks/sprint-00/TASK-0008-documentacao-operacional-fundacao.md) | Documentacao operacional da fundacao. | Planejada | |

## Criterios de aceite da sprint

- Projeto compila localmente.
- Testes base executam.
- Migrações rodam do zero.
- Pipeline inicial configurado.
- Todo codigo da aplicacao fica dentro de `codebase/`.

## Riscos

- Incompatibilidade de versoes da stack.

## Resultado / Review

- A preencher.

## Retrospectiva

- A preencher.
